...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'OF8AV0tf15HMyD_',
      'username':'Norwood_Welch97',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Norwood_Welch97' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      'password':'Sjp8ej3Qag2xahL',
      'username':'Hillary.Hoppe',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Hillary.Hoppe' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
